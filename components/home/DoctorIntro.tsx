...
import { doctorIntro } from "@/data/doctorIntro";
import { siteContent } from "@/data/site";
import type { Locale } from "@/lib/i18n";

export function DoctorIntro({ locale }: { locale: Locale }) {
    ...
    <Section tint="neutral" aria-labelledby="intro-heading">
      <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
        ...
        <Reveal>
          ...
          <Button href={`${localeRoot}/about`} variant="ghost" withArrow className="mt-7 !px-0">
            {siteContent.actions.learnMoreAboutDoctor[locale]}
          </Button>
        </Reveal>
        ...
        <Reveal delay={0.15} className="relative">
          ...
          <InlineVideo video={doctorIntro.video} title={doctorIntro.heading} locale={locale} className="relative" />
        </Reveal>
      </div>
    </Section>
  );
